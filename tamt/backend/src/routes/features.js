...
// GET /features
router.get('/', (req, res) => {
  const { status, feature_type, priority, search } = req.query;
  let sql = `
    SELECT f.*,
...
router.get('/:id', (req, res) => {
  ...
  res.json({ ...feature, prerequisites, apiContracts, testCases });
});

// POST /features
router.post('/', (req, res) => {
  const { name, description, feature_type, priority } = req.body;
  if (!name) return res.status(400).json({ error: 'Name is required' });

  const result = db.prepare(`
    INSERT INTO features (name, description, feature_type, priority, status, created_by)
    VALUES (?, ?, ?, ?, 'Draft', ?)
  `).run(name, description || '', feature_type || 'Functional', priority || 'P2', req.user.id);
...
router.put('/:id', (req, res) => {
  const { name, description, feature_type, priority, status } = req.body;
  ...
  db.prepare(`
    ...
    feature_type = COALESCE(?, feature_type), priority = COALESCE(?, priority),
    status = COALESCE(?, status), updated_at = CURRENT_TIMESTAMP
    WHERE id = ?
  `).run(name, description, feature_type, priority, status, req.params.id);
